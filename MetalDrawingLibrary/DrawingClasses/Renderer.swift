...
class Renderer: NSObject, RendererDelegate {
    ...
    func makeTexture() -> MTLTexture {
        ...
        textureDescriptor.width = Int(metalView.viewportSize.x)
        textureDescriptor.height = Int(metalView.viewportSize.y)
    ...
    func render() {
        ...
        renderEncoder.setViewport(MTLViewport(originX: 0.0, originY: 0.0, width: Double(metalView.viewportSize.x), height: Double(metalView.viewportSize.y), znear: 0.0, zfar: 1.0))
